...
namespace Elim.Event
{
  public class PCEvent : ControllerP<CMEvent>
  {
    ...
    #region Register
    public bool RegisterAssistance(Guid EventId, Guid? ChurchId, string Name, string Surnames, string Email, string Facebook, DateTime? Birthday, decimal Cooperation)
    {
      Model.Event = DBEvent.Get(EventId);
      if (PCYoung.SaveYoung(ChurchId, Name, Surnames, Email, Facebook, Birthday))
      {
